<script lang="ts">
    import { REMEMBERED_LOGIN, storageGet, storageSet, storageRemove, readRememberedLogin, rememberLogin, clearLogin, localRedirect, rememberedSignIn } from '$lib/utils/rememberedLogin';
    let rememberMe = true;
	...
	const setSessionUser = async (sessionUser, redirectPath: string | null = null) => {
		if (sessionUser) {
			...
			if (sessionUser.token) {
				storageSet('token', sessionUser.token);
			}
			...
			goto(localRedirect(redirectPath));
			storageRemove('redirectPath');
		}
	};

	const signInHandler = async (automatic = false) => {
		const sessionUser = await rememberedSignIn(email, password).catch((error) => {
			toast.error(`${error}`);
			return null;
		});

		if (sessionUser && !automatic) rememberLogin(email, password, rememberMe);
    ...
    onMount(async () => {
        ...
        if (logout) { clearLogin(false); await user.set(undefined); }
        else {
            // Validate native token/cookie before sending a remembered password.
            const session = await getSessionUser(storageGet('token') || '').catch(() => null);
            if (session) { await setSessionUser(session); return; }
            storageRemove('token');
            const saved = readRememberedLogin();
            if (saved) { email = saved.email; password = saved.password; await signInHandler(true); }
        }
        loaded = true;
    });
</script>
					...
					<div class="my-auto flex flex-col justify-center items-center">
						<div id="auth-login-card" class=" sm:max-w-md my-auto pb-10 w-full dark:text-gray-100">
							...
							>
								...
								<div class="mt-5">
									{#if $config?.features.enable_login_form || $config?.features.enable_ldap || form}
										<label class="flex items-center gap-2 text-sm my-3">
                                                <input type="checkbox" bind:checked={rememberMe} /> Remember me on this browser
                                            </label>
                                            <p class="text-xs opacity-60 mb-3">Saves your email and password in this browser. Use only on a trusted device.</p>
                                            {#if mode === 'ldap'}
											<button
