<script>
    import { LOGOUT_EVENT, storageRemove, REMEMBERED_LOGIN } from '$lib/utils/rememberedLogin';
    onMount(() => {
        /** @param {StorageEvent} event */
        const logoutSync = (event) => {
            if (event.key === LOGOUT_EVENT) {
                storageRemove('token'); storageRemove(REMEMBERED_LOGIN);
                window.location.assign('/auth?state=logout');
            }
        };
        window.addEventListener('storage', logoutSync);
        return () => window.removeEventListener('storage', logoutSync);
    });
	...
	const setupSocket = async (enableWebsocket) => {
		...
		_socket.on('connect', async () => {
			...
			const res = localStorage.token ? await getVersion(localStorage.token).catch(() => null) : null;
