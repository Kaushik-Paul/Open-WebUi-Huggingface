<script lang="ts">
	...
	export let connection: { url: string; key: string; config: Record<string, any> } | null = null;
	...
	const submitHandler = async () => {
		...
		key = '';
		auth_type = 'bearer';
		key_source = (direct || ollama) ? 'literal' : 'secret';
		prefixId = '';
		passthroughParams = '';
		showAdvanced = false;
		tags = [];
	...
	const init = () => {
		if (connection) {
			...
			key_source = connection.config.key_source ?? 'literal';
