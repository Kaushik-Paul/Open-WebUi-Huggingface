<script lang="ts">
	...
	let models = null;
	let editModels: { id: string; name: string }[] = [];
	let config = null;
	const inputClass =
	...
	const getModels = async () => {
		editModels = await getImageGenerationModels(localStorage.token, true).catch(() => []);
			...
			<div class="flex flex-col">
				...
				<AdminSettingSection title={$i18n.t('Create Image')}>
					...
					{#if config?.IMAGE_GENERATION_ENGINE === 'openai'}
                        <AdminSettingField label="Provider compatibility">
                            <select class={inputClass} bind:value={config.IMAGES_OPENAI_COMPATIBILITY}>
                                <option value="standard">Standard OpenAI</option><option value="surplus">Surplus</option>
                            </select>
                        </AdminSettingField>
                        <AdminSettingField label="Credential source">
                            <select class={inputClass} bind:value={config.IMAGES_OPENAI_KEY_SOURCE}>
                                <option value="secret">Secret name</option><option value="literal">Literal API key</option>
                            </select>
                        </AdminSettingField>
                        {#if config.IMAGES_OPENAI_KEY_SOURCE === 'secret'}<p class="text-xs opacity-70">Enter SURPLUS_API_KEY. Its value stays on the server.</p>{/if}
                        {#if config.IMAGES_OPENAI_COMPATIBILITY === 'surplus'}<p class="text-xs opacity-70">Use a current Surplus model ID. Save settings to refresh the catalog. Leave API version empty. Masks are unsupported.</p>{/if}
				...
				<AdminSettingSection title={$i18n.t('Edit Image')}>
					...
					{#if config?.IMAGE_EDIT_ENGINE === 'openai'}
                        <AdminSettingField label="Provider compatibility">
                            <select class={inputClass} bind:value={config.IMAGES_EDIT_OPENAI_COMPATIBILITY}>
                                <option value="standard">Standard OpenAI</option><option value="surplus">Surplus</option>
                            </select>
                        </AdminSettingField>
                        <AdminSettingField label="Credential source">
                            <select class={inputClass} bind:value={config.IMAGES_EDIT_OPENAI_KEY_SOURCE}>
                                <option value="secret">Secret name</option><option value="literal">Literal API key</option>
                            </select>
                        </AdminSettingField>
                        {#if config.IMAGES_EDIT_OPENAI_KEY_SOURCE === 'secret'}<p class="text-xs opacity-70">Enter SURPLUS_API_KEY. Its value stays on the server.</p>{/if}
                        {#if config.IMAGES_EDIT_OPENAI_COMPATIBILITY === 'surplus'}<p class="text-xs opacity-70">Use a current Surplus model ID. Save settings to refresh the catalog. Leave API version empty. Masks are unsupported.</p>{/if}
