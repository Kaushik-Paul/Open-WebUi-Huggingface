...
def _clean_proxy_headers(raw_headers) -> dict:
    ...
    return {k: v for k, v in raw_headers.items() if k.lower() in ('content-type', 'cache-control')}
...
async def send_get_request(
    ...
    try:
        async with ProviderSession(timeout=_MODEL_LIST_TIMEOUT, trust_env=True) as session:
...
async def get_models_request(
    ...
    key = connection_credential(url, key, config)
    config = config or {}
...
async def get_openai_connection(idx: int) -> tuple[str, str, dict]:
    ...
    key = connection_credential(url, key, api_config)
...
async def update_config(request: Request, form_data: OpenAIConfigForm, user=Depends(get_admin_user)):
    ...
    for idx, url in enumerate(form_data.OPENAI_API_BASE_URLS):
        connection_credential(url, api_keys[idx], api_configs.get(str(idx), {}), status=400)
...
async def verify_connection(
    ...
    key = connection_credential(url, key, api_config, status=400)
