...
from open_webui.utils.provider_http import get_provider_session as get_session
...
IMAGE_CONFIG_KEYS = {
    ...
    'IMAGES_OPENAI_KEY_SOURCE': 'image_generation.openai.key_source',
    'IMAGES_OPENAI_COMPATIBILITY': 'image_generation.openai.compatibility',
    ...
    'IMAGES_EDIT_OPENAI_KEY_SOURCE': 'images.edit.openai.key_source',
    'IMAGES_EDIT_OPENAI_COMPATIBILITY': 'images.edit.openai.compatibility',
...
async def get_image_model(request):
    ...
    if image_config.IMAGE_GENERATION_ENGINE == 'openai':
        if image_config.IMAGES_OPENAI_COMPATIBILITY == 'surplus':
            return image_config.IMAGE_GENERATION_MODEL
...
class ImagesConfig(BaseModel):
    ...
    IMAGES_OPENAI_KEY_SOURCE: Literal['literal', 'secret'] = 'literal'
    IMAGES_OPENAI_COMPATIBILITY: Literal['standard', 'surplus'] = 'standard'
    ...
    IMAGES_EDIT_OPENAI_KEY_SOURCE: Literal['literal', 'secret'] = 'literal'
    IMAGES_EDIT_OPENAI_COMPATIBILITY: Literal['standard', 'surplus'] = 'standard'
...
async def update_config(request: Request, form_data: ImagesConfig, user=Depends(get_admin_user)):
    for prefix, enabled in [('IMAGES_OPENAI', form_data.ENABLE_IMAGE_GENERATION), ('IMAGES_EDIT_OPENAI', form_data.ENABLE_IMAGE_EDIT)]:
        if enabled:
            connection_credential(getattr(form_data, prefix + '_API_BASE_URL'), getattr(form_data, prefix + '_API_KEY'),
                                  {'key_source': getattr(form_data, prefix + '_KEY_SOURCE')}, status=400)
            if getattr(form_data, prefix + '_COMPATIBILITY') == 'surplus' and getattr(form_data, prefix + '_API_VERSION'):
                raise HTTPException(400, 'Leave API version empty for Surplus')
...
async def verify_url(request: Request, user=Depends(get_admin_user)):
    image_config = await get_image_config()
    if image_config.IMAGE_GENERATION_ENGINE == 'openai' and image_config.IMAGES_OPENAI_COMPATIBILITY == 'surplus':
        await surplus_images.catalog(image_config.IMAGES_OPENAI_API_BASE_URL, image_config.IMAGES_OPENAI_API_KEY,
                                     image_config.IMAGES_OPENAI_KEY_SOURCE)
        return True
...
@router.get('/models')
async def get_models(request: Request, edit: bool = False, user=Depends(get_admin_user)):
    image_config = await get_image_config()
    prefix = 'IMAGES_EDIT_OPENAI' if edit else 'IMAGES_OPENAI'
    if getattr(image_config, prefix + '_COMPATIBILITY') == 'surplus':
        return await surplus_images.catalog(getattr(image_config, prefix + '_API_BASE_URL'),
            getattr(image_config, prefix + '_API_KEY'), getattr(image_config, prefix + '_KEY_SOURCE'), edit=edit)
    try:
        if image_config.IMAGE_GENERATION_ENGINE == 'openai':
            return [
...
async def get_image_data(data: str, headers=None, trusted_base_url: str | None = None):
    try:
        return await surplus_images.image_bytes(data)
    except Exception:
        raise HTTPException(502, 'Invalid or unsafe image result') from None
...
async def image_generations(
    ...
    image_config = await get_image_config()
    if image_config.IMAGE_GENERATION_ENGINE == 'openai' and image_config.IMAGES_OPENAI_COMPATIBILITY == 'surplus':
        body = surplus_images.image_payload(await get_image_model(request), form_data.prompt, form_data.n,
            form_data.size or image_config.IMAGE_SIZE, image_config.IMAGES_OPENAI_API_PARAMS)
        results = await surplus_images.request_images(image_config.IMAGES_OPENAI_API_BASE_URL,
            image_config.IMAGES_OPENAI_API_KEY, image_config.IMAGES_OPENAI_KEY_SOURCE, body)
        return [(await upload_image(request, data, mime, {**body, **(metadata or {})}, user))[1] for data, mime in results]
    ...
    try:
        if image_config.IMAGE_GENERATION_ENGINE == 'openai':
            headers = {
                'Authorization': 'Bearer ' + connection_credential(image_config.IMAGES_OPENAI_API_BASE_URL, image_config.IMAGES_OPENAI_API_KEY, {'key_source': image_config.IMAGES_OPENAI_KEY_SOURCE}),
...
async def image_edits(
    ...
    image_config = await get_image_config()
    surplus = image_config.IMAGE_EDIT_ENGINE == 'openai' and image_config.IMAGES_EDIT_OPENAI_COMPATIBILITY == 'surplus'
    if surplus and (form_data.mask or form_data.mask_url):
        raise HTTPException(400, 'Surplus does not support masks or inpainting')
    if isinstance(form_data.image, list) and len(form_data.image) > 8:
        raise HTTPException(400, 'At most 8 source images are supported')
    size = None
    ...
    try:
        ...
        async def load_url_image(data):
            ...
            if data.startswith('http://') or data.startswith('https://'):
                ...
                image_data, mime = await surplus_images.download_image(data)
                ...
                if isinstance(file_response, FileResponse):
                    ...
                    async with aiofiles.open(file_path, 'rb') as f:
                        file_bytes = await f.read(surplus_images.MAX_IMAGE_BYTES + 1)
                    surplus_images.validate_image(file_bytes)
    ...
    if surplus:
        ...
        body = surplus_images.image_payload(model, form_data.prompt, 1, size, sources=sources)
        results = await surplus_images.request_images(image_config.IMAGES_EDIT_OPENAI_API_BASE_URL,
            image_config.IMAGES_EDIT_OPENAI_API_KEY, image_config.IMAGES_EDIT_OPENAI_KEY_SOURCE, body, edit=True)
        # Source image bytes do not belong in file metadata or chat exports.
        safe_metadata = {k: v for k, v in body.items() if k not in ('image', 'input_images')}
        return [(await upload_image(request, data, mime, {**safe_metadata, **metadata}, user))[1] for data, mime in results]

    try:
        if image_config.IMAGE_EDIT_ENGINE == 'openai':
            headers = {
                'Authorization': 'Bearer ' + connection_credential(image_config.IMAGES_EDIT_OPENAI_API_BASE_URL, image_config.IMAGES_EDIT_OPENAI_API_KEY, {'key_source': image_config.IMAGES_EDIT_OPENAI_KEY_SOURCE}),
            }
