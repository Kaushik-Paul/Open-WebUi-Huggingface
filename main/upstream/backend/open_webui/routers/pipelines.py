...
async def get_openai_connection(url_idx: int) -> tuple[str, str]:
    from open_webui.routers.openai import get_openai_connection as connection
    url, key, _ = await connection(url_idx)
    return url, key
...
async def process_pipeline_inlet_filter(request, payload, user, models):
    ...
    async with ProviderSession(trust_env=True) as session:
