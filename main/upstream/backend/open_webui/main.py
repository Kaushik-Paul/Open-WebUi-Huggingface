...
async def lifespan(app: FastAPI):
    ...
    from open_webui.utils.private_deployment import bootstrap_owner
    ...
    await close_session()
    from open_webui.utils.provider_http import close_provider_session
...
async def unload_model(request: Request, form_data: ModelUnloadForm, user=Depends(get_admin_user)):
    ...
    if model_id in openai_models:
        ...
        key = connection_credential(base_url, key, api_config)
        ...
        if provider == 'llama.cpp':
            ...
            try:
                timeout = aiohttp.ClientTimeout(total=30)
                async with ProviderSession(timeout=timeout, trust_env=True) as session:
                    headers = {
                        'Content-Type': 'application/json',
                        **({'Authorization': f'Bearer {key}'} if key else {}),
                    }
...
async def get_app_config(request: Request):
    ...
    if token:
        ...
        try:
            user = await get_current_user(request, Response(), BackgroundTasks(),
...
# Outermost HTTP guard also covers upstream public file/share routes.
from open_webui.utils.provider_http import ProviderSession
from open_webui.utils.private_deployment import PrivateAccessMiddleware
