...
from open_webui.utils.private_deployment import POLICY
...
class Config(Base):
    ...
    def configure(
        ...
        *,
        ...
        cls.DEFAULTS = {**(defaults or {}), **POLICY}
    ...
    def default_value(cls, key: str, default: Any = None) -> Any:
        return POLICY.get(key, cls.DEFAULTS.get(key, default))
    ...
    async def get_many(*keys: str) -> dict:
        ...
        disabled_values = {
            ...
            for key in keys
            if not Config.persistent_enabled_for(key) and (key in Config.DEFAULTS or key in POLICY)
        }
    ...
    async def get_namespace(namespace: str) -> dict:
        ...
        async with get_async_db() as db:
            ...
            values.update({key: value for key, value in POLICY.items() if key in values})
            return values
    ...
    async def upsert(updates: dict) -> None:
        ...
        updates = {key: POLICY.get(key, value) for key, value in updates.items()}
        persistent_updates = {}
